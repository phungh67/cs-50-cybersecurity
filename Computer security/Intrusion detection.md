...
With various ways to attack (mentioned in [[Attacks]] and [[Attacking types]]) and with the consequences from crash and kill to the exhaustive the system [[Cybersecurity/cs-50-cybersecurity/Computer security/Denial of Service attack]], we want to not only build a system that can mitigate these threats, but also want to build some early warning, alerting stack to detect the abnormal activities.
