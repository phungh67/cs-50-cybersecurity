...
- Launching a `DDoS` (distributed denial of service attack) is not too hard, in fact, you can hire a fleet of machines to do this just with 5$ per hour.
- This is an "active" attack, that is, easy to detect but very hard to completely mitigate it, so the goal is to lessen the impact and damage as much as possible. Thus, it already costs thousands if not billions of dollar to completely apply protection mechanisms.

Moreover, a down time, no matter how long, can affect severely to the owner's business.

# Early classification

In the category of `DoS` attack, there are several types to consider:
- Classic Denial of Service attack - aim to disrupt a service completely, making legitimate users cannot access the resources.
...
For short: Denial of Service is an attack that makes legitimate users cannot enter nor being severed by the system, causing chaos and damaging the user's experience.
...
- System resource: simply enough, trying to consume as much as possible all resources from memory, disk space,... or even intentionally sending a malicious packet to trigger hidden bugs.
...
But the thing is, the last message will never come because the attackers will modify the source IP address so that the victim will always wait for reply from "invalid" address, hence, keeping the connection open, consuming the slot in the Transmission Control Board.
...
Further reading [[Transport layer security]] and [[Lab 1 - Nmap and WireShark]] (for reconnaissance )

**Mitigation strategy - RFC 4987**

**`SYN` cookies**

Short description, the server challenges the packet if it was from a flood before accepting it. Legitimate client will complete the challenge without any difficulties. while the flood could not do since the intention of attacker is never to complete the handshake.
...
- Multi layers of defenses are recommended:
    ...
    - `SYN` cookies and `eBPF` reduces memory exhaustion, not for network congestion, but this is the last layer.