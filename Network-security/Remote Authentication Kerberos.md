...
Related to user authentication problem : [[User authentication]]
...
**Kerberos server**: contains Key Distribution Center, Authentication Server and Authorization server (this is how components are grouped in the Kerberos, not the actual implementation in real world).
...
We know that master key is a key used to derived all sub-keys in the process. In this scenario, master key $K_a$ is a hash of the user's password.
